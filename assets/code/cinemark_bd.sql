...
CREATE DATABASE IF NOT EXISTS cinemark_bd CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_general_ci' ;
...
CREATE TABLE IF NOT EXISTS funcion (
  ...
  PRIMARY KEY (id),
  FOREIGN KEY (pelicula_id) REFERENCES pelicula (id) ON UPDATE CASCADE ON DELETE RESTRICT,
  FOREIGN KEY (sala_id) REFERENCES sala (id) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE = INNODB ;
...
CREATE TABLE IF NOT EXISTS tarjeta (
  ...
  habilitada BOOL NOT NULL DEFAULT TRUE,
  PRIMARY KEY (id),
  UNIQUE dni_cliente (dni_cliente),
...
CREATE TABLE IF NOT EXISTS ticket (
  ...
  PRIMARY KEY (codigo),
  FOREIGN KEY (funcion_id) REFERENCES funcion (id) ON UPDATE CASCADE ON DELETE RESTRICT,
  FOREIGN KEY (dni_cliente) REFERENCES cliente (dni) ON UPDATE CASCADE ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS reserva (
  ...
  funcion_id INTEGER NOT NULL,
  butacas TINYINT NOT NULL,
  fecha TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (dni_cliente, funcion_id),
  FOREIGN KEY (dni_cliente) REFERENCES cliente (dni) ON UPDATE CASCADE ON DELETE CASCADE,
  FOREIGN KEY (funcion_id) REFERENCES funcion (id) ON UPDATE CASCADE ON DELETE CASCADE
) ;
...
WHERE pelicula.cartelera = TRUE 
  AND sala.habilitada = TRUE 
  AND (
    DAY(funcion.fin) = DAY(NOW()) 
...
(SELECT 
  ...
  FROM
    ticket 
  WHERE funcion_id = funcion.id) AS 'Butacas vendidas',
  (SELECT 
    sala.capacidad - 
    (SELECT 
      COUNT(*) 
  ...
  FROM
    sala 
  WHERE sala.id = funcion.sala_id) AS 'Butacas disponibles',
  precio AS 'Precio' 
FROM
  (
    (
      (
        sala 
        LEFT JOIN funcion 
          ON sala.id = funcion.sala_id
      ) 
      INNER JOIN pelicula 
        ON pelicula.id = funcion.pelicula_id
    ) 
    INNER JOIN ticket 
      ON ticket.funcion_id = funcion.id
  ) 
...
(SELECT 
  dni_cliente AS 'Cliente',
  pelicula.titulo AS 'Película',
  sala.nombre AS 'Sala',
  butacas AS 'Asientos reservados',
  fecha AS 'Fecha de reserva' 
FROM
  (
    (
      pelicula 
    ...
    RIGHT JOIN sala 
      ON funcion.sala_id = sala.id
  ) 
  INNER JOIN reserva 
    ON reserva.funcion_id = funcion.id) ;
...
BEGIN
  ...
  FROM
    funcion 
  WHERE pelicula_id = @_pelicula_id_ 
...
CREATE PROCEDURE listarPeliculas (arg0 INTEGER, arg1 INTEGER) 
BEGIN
  SELECT 
    * 
  FROM
...
CREATE PROCEDURE listarSalas (arg0 INTEGER, arg1 INTEGER) 
BEGIN
  SELECT 
    * 
  FROM
...
CREATE PROCEDURE listarFunciones (arg0 INTEGER, arg1 INTEGER) 
BEGIN
  SELECT 
    * 
  FROM
...
DROP PROCEDURE IF EXISTS listarFuncionesEnVenta ;

DELIMITER //

CREATE PROCEDURE listarFuncionesEnVenta (arg0 INTEGER, arg1 INTEGER) 
BEGIN
  SELECT 
    * 
  FROM
...
CREATE PROCEDURE agregarCliente (arg0 VARCHAR (10), arg1 DATE, arg2 DATE) 
BEGIN
  INSERT INTO cliente (dni, fechaNac, alta) 
  VALUES
    (arg0, arg1, arg2) ;
END //
...
BEGIN
  ...
  CALL idFuncionPorPeliculaYSala (arg1, arg2, funcion_id_) ;
  INSERT INTO ticket (
    codigo,
    funcion_id,
    butaca,
  ...
  VALUES
    (arg0, funcion_id_, arg3, arg4, arg5) ;
END //
...
BEGIN
  INSERT INTO tarjeta (alta, baja, dni_cliente) 
  VALUES
    (arg0, arg1, arg2) ;
END //
...
DROP PROCEDURE IF EXISTS agregarReserva ;
...
BEGIN
  CALL idFuncionPorPeliculaYSala (arg0, arg1, @_funcion_id_) ;
  INSERT INTO reserva (dni_cliente, funcion_id, butacas) 
  VALUES
    (arg2, @_funcion_id_, arg3) ;
END //
...
BEGIN
  INSERT INTO usuario (dni_cliente, nombre, clave, modulo) 
  VALUES
    (arg0, arg1, arg2, arg3) ;
END //
...
BEGIN
  CALL idSalaPorNombre (arg0, @_id_) ;
  UPDATE 
    sala 
  SET
    nombre = arg1,
...
BEGIN
  CALL idFuncionPorPeliculaYSala (arg1, arg2, @_funcion_id_) ;
  UPDATE 
    ticket 
  SET
...
CREATE PROCEDURE borrarFuncion (arg0 VARCHAR (20), arg1 VARCHAR (120)) 
BEGIN
  CALL idFuncionPorPeliculaYSala (arg0, arg1, @_id_) ;
  DELETE 
  FROM
    funcion 
  WHERE id = @_id_ ;
...
CREATE PROCEDURE borrarCliente (arg0 VARCHAR (10)) 
BEGIN
  DELETE 
  FROM
    cliente 
...
CREATE PROCEDURE borrarUsuario (arg0 VARCHAR (11)) 
BEGIN
  DELETE 
  FROM
    usuario 
...
DROP PROCEDURE IF EXISTS borrarReserva ;

DELIMITER //

CREATE PROCEDURE borrarReserva (arg0 VARCHAR (10)) 
BEGIN
  DELETE 
  FROM
    reserva 
...
DROP PROCEDURE IF EXISTS borrarTarjeta ;

DELIMITER //

CREATE PROCEDURE borrarTarjeta (arg0 VARCHAR (10)) 
BEGIN
  DELETE 
  FROM
    tarjeta 
...
CREATE PROCEDURE logearUsuario (arg0 VARCHAR (30), arg1 VARCHAR (8)) 
BEGIN
  SELECT 
    modulo 
  FROM
    (
      usuario 
      LEFT JOIN cliente 
        ON cliente.dni = usuario.dni_cliente
    ) 
  WHERE activo = TRUE 
    AND (nombre = arg0 
      AND clave = arg1) 
...
DROP PROCEDURE IF EXISTS verificarConsumo ;

DELIMITER //

CREATE PROCEDURE verificarConsumo (arg0 VARCHAR (11)) 
BEGIN
  SELECT 
    COUNT(dni_cliente) AS 'Tickets vendidos',
    MONTH(fechaVenta) AS 'Mes' 
  FROM
    ticket 
  WHERE dni_cliente = arg0 
    AND YEAR(fechaVenta) = YEAR(NOW()) 
  GROUP BY MONTH(fechaVenta) ;
END //

DELIMITER ;

DROP PROCEDURE IF EXISTS purgarCliente ;

DELIMITER //

CREATE PROCEDURE purgarCliente (arg0 VARCHAR (10)) 
BEGIN
  DELETE 
  FROM
    ticket 
...
DROP PROCEDURE IF EXISTS purgarPelicula ;

DELIMITER //

CREATE PROCEDURE purgarPelicula (arg0 VARCHAR (20)) 
BEGIN
  CALL idPeliculaPorIMDB (arg0, @_id_) ;
  DELETE 
  FROM
  ...
  WHERE 
    ...
    FROM
      ticket 
      INNER JOIN funcion 
        ON funcion.id = ticket.funcion_id 
    WHERE funcion.pelicula_id = @_id_) ;
  DELETE 
  FROM
    funcion 
  WHERE pelicula_id = @_id_ ;
END //

DELIMITER ;

DROP PROCEDURE IF EXISTS purgarSala ;

DELIMITER //

CREATE PROCEDURE purgarSala (arg0 VARCHAR (120)) 
BEGIN
  CALL idSalaPorNombre (arg0, @_id_) ;
  DELETE 
  FROM
...
DROP PROCEDURE IF EXISTS purgarFuncion ;

DELIMITER //

CREATE PROCEDURE purgarFuncion (arg0 VARCHAR (20), arg1 VARCHAR (120)) 
BEGIN
  CALL idFuncionPorPeliculaYSala (arg0, arg1, @_id_) ;
  DELETE 
  FROM
    ticket 
  WHERE funcion_id = @_id_ ;
END //
